/**
 * Phase 1-3 v2: Optimized — batch aggregate counts instead of per-product queries.
 */
...
console.log(`Parsed ${adjustedProducts.length} product entries`)

// ============ PHASE 2: LOAD METALTRACK (batch) ============
console.log('\n=== PHASE 2: LOAD METALTRACK PRODUCTS (batch) ===')
const mtProductsRaw = await db.product.findMany({
  include: { category: true },
  orderBy: [{ categoryId: 'asc' }, { sortOrder: 'asc' }],
})

// Batch aggregate stock + movement counts using groupBy
...
console.log(`Loaded ${mtProducts.length} MT products with stock + movement info`)
...
function findMatch(adj) {
  ...
  // Contains match within same category
  const containsSameCat = mtProducts.filter(p => p.categoryName === adj.mtCategory && (p.normName.includes(adj.normName) || adj.normName.includes(p.normName)))
  if (containsSameCat.length === 1) {
    const sim = similarity(adj.normName, containsSameCat[0].normName)
    return { product: containsSameCat[0], matchType: 'RENAME_EXISTING', confidence: sim.score >= 0.8 ? 'high' : 'medium', reason: `contains-match (sim ${(sim.score*100).toFixed(0)}%): "${containsSameCat[0].name}" → "${adj.rawName}"` }
  }
  
  // Fuzzy within same category
  const candidates = mtProducts.filter(p => p.categoryName === adj.mtCategory).map(p => ({ product: p, sim: similarity(adj.normName, p.normName) })).filter(c => c.sim.score >= 0.7).sort((a, b) => b.sim.score - a.sim.score)
  if (candidates.length > 0) {
    return { product: candidates[0].product, matchType: 'RENAME_EXISTING', confidence: candidates[0].sim.score >= 0.85 ? 'high' : 'medium', reason: `fuzzy match (sim ${(candidates[0].sim.score*100).toFixed(0)}%): "${candidates[0].product.name}" → "${adj.rawName}"` }
  }
  return null
}
