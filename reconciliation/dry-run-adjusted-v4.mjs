/**
 * v4: Fixed matching priority:
 * 1. Exact name + same category → EXACT_MATCH
 * 2. Exact name, different category → CATEGORY_CHANGE
 * 3. Contains-match (adjusted is substring of MT OR MT is substring of adjusted, same category)
 *    → Pick the LONGEST MT name that contains the adjusted name (most specific match)
 *    → RENAME_EXISTING (high confidence)
 * 4. Fuzzy match (same category, sim >= 0.85) → RENAME_EXISTING (medium confidence)
 * 5. No match → CREATE_NEW_PRODUCT
 */
...
function findMatch(adj) {
  ...
  // 3. Contains-match within same category (HIGHEST PRIORITY for rename detection)
  // For contains check, also try with punctuation stripped (handles "304 ยาว" vs "304 (ยาว)")
  const stripped = s => s.replace(/[\s,\-_/()]/g, '')
  ...
  }).sort((a, b) => b.sim.score - a.sim.score)  // among contains matches, pick highest similarity
  ...
  const fuzzyCandidates = mtProducts.filter(p => p.categoryName === adj.mtCategory).map(p => ({ product: p, sim: similarity(adj.normName, p.normName) })).filter(c => c.sim.score >= 0.85).sort((a, b) => b.sim.score - a.sim.score)
