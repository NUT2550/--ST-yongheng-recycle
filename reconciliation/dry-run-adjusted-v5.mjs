/**
 * v5: Owner correction — สายไฟอลูมิเนียม (insulated) and อลูมิเนียมสายไฟ (stripped)
 *     are DIFFERENT products. They must NOT match each other.
 *
 * Added EXACT_EXCLUSIONS list: pairs of names that must never be matched together
 * even if they have high fuzzy/contains similarity.
 */
...
function normalize(s) {
  ...
  let t = fixThai(s)
  t = t.replace(/[\u200B-\u200D\uFEFF\u00A0]/g, ' ')
  t = t.replace(/\s+/g, ' ').trim().normalize('NFC')
  return t
}

// OWNER-APPROVED EXACT EXCLUSIONS — these name pairs must NEVER match each other
// even if fuzzy/contains similarity is high. They are genuinely different products.
const EXACT_EXCLUSIONS = [
  {
    name1: 'สายไฟอลูมิเนียม',      // insulated aluminum cable (with sheath)
    name2: 'อลูมิเนียมสายไฟ',      // stripped aluminum wire (pure aluminum)
    reason: 'Owner correction (Task 39): สายไฟอลูมิเนียม (insulated cable, with rubber/plastic sheath) is DIFFERENT from อลูมิเนียมสายไฟ (stripped wire, pure aluminum). Do NOT rename/merge/alias.',
  },
]

function isExcludedPair(name1, name2) {
  const n1 = normalize(name1)
  const n2 = normalize(name2)
  for (const ex of EXACT_EXCLUSIONS) {
    const e1 = normalize(ex.name1)
    const e2 = normalize(ex.name2)
    if ((n1 === e1 && n2 === e2) || (n1 === e2 && n2 === e1)) {
      return ex.reason
    }
  }
  return null
...
function findMatch(adj) {
  ...
  // 3. Contains-match within same category
  //    BUT: skip candidates that are in EXACT_EXCLUSIONS with the adjusted name
  const stripped = s => s.replace(/[\s,\-_/()]/g, '')
  const adjStripped = stripped(adj.normName)
  
  const containsCandidates = mtProducts.filter(p => {
    if (p.categoryName !== adj.mtCategory) return false
    // Check exclusion list — if this MT product is in an exclusion pair with adjusted name, skip
    const exclusionReason = isExcludedPair(adj.normName, p.name)
    if (exclusionReason) return false  // never match excluded pairs
    const pStripped = stripped(p.normName)
    return p.normName.includes(adj.normName) || adj.normName.includes(p.normName) || adjStripped.includes(pStripped) || pStripped.includes(adjStripped)
  }).map(p => {
    const sim = similarity(adj.normName, p.normName)
    const pStripped = stripped(p.normName)
    const containsMatch = p.normName.includes(adj.normName) || adj.normName.includes(p.normName)
    const containsStripped = adjStripped.includes(pStripped) || pStripped.includes(adjStripped)
    return { product: p, sim, containsMatch, containsStripped }
  }).sort((a, b) => b.sim.score - a.sim.score)
  
  if (containsCandidates.length > 0) {
    const best = containsCandidates[0]
    const matchMethod = best.containsMatch ? 'contains-match' : 'punctuation-insensitive contains'
    return { product: best.product, matchType: 'RENAME_EXISTING', confidence: best.sim.score >= 0.7 ? 'high' : 'medium', reason: `${matchMethod} in ${adj.mtCategory} (sim ${(best.sim.score*100).toFixed(0)}%): "${best.product.name}" → "${adj.rawName}"` }
  }
  
  // 4. Fuzzy match within same category (sim >= 0.85) — only if no contains match
  //    BUT: skip candidates in EXACT_EXCLUSIONS
  const fuzzyCandidates = mtProducts.filter(p => {
    if (p.categoryName !== adj.mtCategory) return false
    const exclusionReason = isExcludedPair(adj.normName, p.name)
    if (exclusionReason) return false
    return true
  }).map(p => ({ product: p, sim: similarity(adj.normName, p.normName) })).filter(c => c.sim.score >= 0.85).sort((a, b) => b.sim.score - a.sim.score)
  if (fuzzyCandidates.length > 0) {
    const best = fuzzyCandidates[0]
    return { product: best.product, matchType: 'RENAME_EXISTING', confidence: 'medium', reason: `fuzzy match in ${adj.mtCategory} (sim ${(best.sim.score*100).toFixed(0)}%): "${best.product.name}" → "${adj.rawName}"` }
  }
...
const results = []
const exclusionHits = []  // track when exclusion was triggered
for (const adj of adjustedProducts) {
  const r = { ...adj, classification: null, matchedProduct: null, confidence: null, reason: null, exclusionTriggered: null }
  ...
  if (match) {
    ...
    // Check if exclusion was the reason for no match
    for (const p of mtProducts) {
      const exclReason = isExcludedPair(adj.normName, p.name)
      if (exclReason) {
        r.exclusionTriggered = { mtProductName: p.name, reason: exclReason }
        exclusionHits.push({ adjustedName: adj.rawName, mtProductName: p.name, reason: exclReason })
      }
    }
    ...
    if (sameNameInAdjusted.length > 1) {
      ...
      if (r.exclusionTriggered) {
        r.reason = `No match (excluded from matching with "${r.exclusionTriggered.mtProductName}"): ${r.exclusionTriggered.reason}`
      } else {
...
fs.writeFileSync('/home/z/my-project/reconciliation/dry-run-results-v5.json', JSON.stringify({
...
// Re-apply owner review flags
function similarity2(a, b) { return similarity(a, b).score }
...
for (const p of results) {
  ...
  if (p.classification === 'RENAME_EXISTING') {
    const matchedProduct = mtProducts.find(m => m.id === p.matchedProduct.id)
    if (matchedProduct) {
      const betterCandidates = mtProducts.filter(m =>
        m.categoryName === p.mtCategory &&
        m.id !== p.matchedProduct.id &&
        !isExcludedPair(p.normName, m.name) &&  // respect exclusions
        similarity2(p.normName, m.normName) >= 0.9
      )
      if (betterCandidates.length > 0) {
        flags.push(`CLOSER_MATCH_EXISTS: "${betterCandidates[0].name}" has fuzzy sim ${(similarity2(p.normName, betterCandidates[0].normName)*100).toFixed(0)}% but was not selected`)
      }
    }
  }
  if (p.classification === 'RENAME_EXISTING' && p.matchedProduct.stockWeight >= 100) {
    flags.push(`SIGNIFICANT_STOCK: matched product has ${p.matchedProduct.stockWeight} kg stock — verify rename is safe`)
  }
  if (p.classification === 'CATEGORY_CHANGE' && (p.matchedProduct.hasStockLots || p.matchedProduct.hasMovement)) {
    flags.push(`CATEGORY_CHANGE_WITH_DATA: product has stock/movement — changing category is risky`)
  }
  if (flags.length > 0) { p.ownerReviewFlags = flags; ownerReviewCount++ }
}

// Save final results (with owner review flags)
fs.writeFileSync('/home/z/my-project/reconciliation/dry-run-results.json', JSON.stringify({
  ...
  adjustedProducts: results.map(r => ({
    ...
    matchedProductHasMovement: r.matchedProduct?.hasMovement ?? null,
    exclusionTriggered: r.exclusionTriggered,
    ownerReviewFlags: r.ownerReviewFlags || null,
  })),
  mtProducts: mtProducts.map(p => ({
    id: p.id, name: p.name, normName: p.normName, categoryId: p.categoryId,
    categoryName: p.categoryName, defaultBuyPrice: p.defaultBuyPrice, sortOrder: p.sortOrder,
...
const counts = {
  ...
  OWNER_REVIEW_REQUIRED: ownerReviewCount + regular.filter(p => p.classification === 'CATEGORY_CHANGE').length,
}
console.log('=== SUMMARY (v5 — with EXACT_EXCLUSIONS) ===')
console.log(JSON.stringify(counts, null, 2))
console.log(`\nExclusion hits: ${exclusionHits.length}`)
for (const h of exclusionHits) {
  console.log(`  ⚠️  "${h.adjustedName}" was EXCLUDED from matching with "${h.mtProductName}"`)
