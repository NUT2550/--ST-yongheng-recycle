/**
 * v3 comparison: post-cleanup state.
 *
 * Changes from v2:
 * - MetalTrack products have been renamed (อลูมีเนียม → อลูมิเนียม) — 21 products
 * - 10 new products created in aluminum category
 * - Safe alias fixed in detailed-excel-import-dialog.tsx
 * - Nickel mapping decision applied:
 *   - old group 05 นิกเกิล → MT นิกเกิล(สแตนเลส)
 *   - old group 08 นิกเกิล → IGNORE (duplicate)
 * - Apply อลูมีเนียม → อลูมิเนียม normalization to old/source names too (per owner Decision 1)
 */
...
function normalize(s) {
  ...
  // Standardize aluminum spelling per owner Decision 1
...
// Nickel mapping decision (owner Decision 4)
// - old group 05 นิกเกิล → MT นิกเกิล(สแตนเลส)
// - old group 08 นิกเกิล → IGNORE (duplicate)
const NICKEL_OVERRIDE = {
  '05_นิกเกิล': { targetProductName: 'นิกเกิล(สแตนเลส)', action: 'map' },
  '08_นิกเกิล': { action: 'ignore', reason: 'duplicate of group 05 นิกเกิล (owner Decision 4)' },
}
...
for (let i = 0; i < rows.length; i++) {
  ...
  if (fixed[1] && /^\d{4,5}$/.test(String(fixed[1]).trim()) && fixed[2]) {
    ...
    if (currentGroup === '11' && code.length === 5) {
      originalGroupCode = '0' + code.substring(2, 3)  // e.g. "11201" → "02", "11501" → "05"
      originalGroupName = ({
        '01': 'เหล็ก', '02': 'อลูมิเนียม', '03': 'ทองแดง', '04': 'ทองเหลือง',
...
for (const op of oldProducts) {
  const r = { ...op, match: null, matchType: null, matchStatus: null, problemType: null, recommendedAction: null, reason: null, confidence: null, candidates: null, nickelOverride: false }

  if (op.isService) {
    r.matchStatus = 'SERVICE_NOT_PRODUCT'
    r.problemType = 'service'
    r.recommendedAction = 'do not import (service, not a product)'
    r.reason = 'Old/source entry is a weighing service (ชั่งรถ) — not a metal/product.'
    r.confidence = 'high'
    results.push(r)
    continue
  }

  // Nickel mapping override (owner Decision 4)
  const nickelKey = `${op.groupCode}_${op.normName}`
  if (NICKEL_OVERRIDE[nickelKey]) {
    const override = NICKEL_OVERRIDE[nickelKey]
    r.nickelOverride = true
    if (override.action === 'map') {
      const targetProduct = mtProducts.find(p => p.name === override.targetProductName)
      if (targetProduct) {
        r.match = targetProduct
        r.matchType = 'NICKEL_OVERRIDE'
        r.matchStatus = 'MATCHED'
        r.reason = `Nickel mapping (owner Decision 4): old group ${op.groupCode} "${op.rawName}" → MT "${targetProduct.name}"`
        r.confidence = 'high'
        results.push(r)
        continue
      }
    } else if (override.action === 'ignore') {
      r.matchStatus = 'IGNORE_DUPLICATE'
      r.problemType = 'duplicate'
      r.recommendedAction = 'ignore old duplicate (owner Decision 4)'
  ...
  if (op.isSortedOutput) {
    ...
    if (m) {
      ...
      r.reason = `Sorted-output entry (orig group ${op.originalGroupCode}/${op.originalGroupName}). Stripped name "${stripped}" matches "${m.product.name}".`
      r.confidence = 'high'
    } else {
...
fs.writeFileSync('/home/z/my-project/reconciliation/comparison-results-v3.json', JSON.stringify({
  oldProducts: results.map(r => ({
    ...
    nickelOverride: r.nickelOverride || false,
  ...
  oldDuplicateGroups: oldDuplicateGroups.map(([name, arr]) => ({
    name,
    entries: arr.map(r => ({ oldCode: r.oldCode, groupCode: r.groupCode, groupName: r.groupName, rowIdx: r.rowIdx, matchStatus: r.matchStatus })),
  })),
...
console.log('=== v3 SUMMARY (post-cleanup) ===')
...
console.log(`  ALIAS (safe/contains): ${regular.filter(r => r.matchType === 'ALIAS' || r.matchType === 'ALIAS_CONTAINS').length}`)
console.log(`  NICKEL_OVERRIDE (mapped): ${regular.filter(r => r.matchType === 'NICKEL_OVERRIDE').length}`)
console.log(`  IGNORE_DUPLICATE: ${regular.filter(r => r.matchStatus === 'IGNORE_DUPLICATE').length}`)
...
console.log(`  SORTED_DUPLICATE: ${sorted.filter(r => r.matchStatus === 'SORTED_DUPLICATE').length}`)
console.log(`  UNMATCHED: ${sorted.filter(r => r.matchStatus === 'UNMATCHED').length}`)
console.log(`\nOld/source duplicate groups: ${oldDuplicateGroups.length}`)
console.log(`MT near-duplicates (>=85%): ${mtNearDupes.length}`)
