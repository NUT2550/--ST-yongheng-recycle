...
For those claims, reload `process/CURRENT_STATE.md`, current code / tests, GitHub / CI, and Production evidence when authorized.
...
- generation must be concurrency-safe enough for the actual write path — do **not** rely on a historical algorithm description (e.g. “count of bills in that year + 1”) if current code has changed. Verify the actual sequence-derivation algorithm from current `src/lib/bill-helpers.ts` / tests before relying on it.
...
- use the current application / service flow (e.g. `DELETE /api/{buy,sell,sorting}-bills/{id}`) rather than ad-hoc Production SQL for normal operations.
...
A sell cancellation must restore the stock / cost effect using authoritative evidence from the original sale / reversal model and preserve audit history. Credit entries linked to the cancelled sale must be reconciled under the current approved implementation.
...
Exact error codes, service functions, and transaction implementation are technical contracts and must be verified from current code / tests (`src/lib/sorting-cancellation-service.ts` and its tests).
...
Known business distinctions (Owner-approved):
...
- `RECEIVABLE` = money a customer owes the business, generally originating from a credit sale.
...
- authentication failure and permission denial must remain distinguishable for UX / security where the current API contract requires it (current contract: `401 AUTH_REQUIRED` vs `403 PERMISSION_DENIED`);
...
- specific internal query algorithms (e.g. bill-number sequence derivation)

Those belong in code / tests, `process/CURRENT_STATE.md`, `process/DATABASE_CONTEXT.md`, `knowledge/` records, and current GitHub evidence.

## Key takeaway

**Keep this file about durable Owner / business behavior. Verify implementation separately from current code / tests / `CURRENT_STATE.md`, and never let a dated code snapshot or implementation-status annotation silently become a business rule.**
